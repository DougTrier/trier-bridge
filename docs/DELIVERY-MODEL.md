...
|---|---|---|
| **Bridge window** | The application: launcher/search home, familiar file entry points with drive letters, settings routing, system tools (Task Manager, Installed Apps, Network Connections with adapter and IPv4 changes, Device Manager, Disk Management, Event Viewer, Services, Printers), Bridge Terminal (`netsh` included), generated Help, and the Integrations page. One unprivileged process per user session. | When the user opens it. |
| **Tray icon "T"** | A StatusNotifier/AppIndicator icon whose menu opens the window, Task Manager, the Command Prompt, the Integrations page, and turns itself off (the taskbar right-click habit). | Only if the user selected it at setup; then it starts at login through a per-user XDG autostart entry. |
