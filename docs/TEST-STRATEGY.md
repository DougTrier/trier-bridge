...
|---|---|---|
| GitHub-hosted disposable `ubuntu-24.04` (CI-01, approved 2026-09-22) | static/security checks, clean package builds, reviewed non-destructive unit subset only | TOOL and selected UNIT evidence; no desktop, mutation, fault-injection, or distribution qualification |
...
| 37.1 repeated execution, cancellation | every plan is a fresh identity check; dialog Cancel paths in the terminal are exercised by hand (entries IMP-03.03, IMP-06.04); the End task dialog specifically was hand-confirmed live 2026-09-21 (shown, read, confirmed, verified) after an automated attempt over AT-SPI found it renders empty from an SSH-launched test process â€” an artifact of that launch method, not the dialog, per entry IMP-06.04's addenda | partly manual |
