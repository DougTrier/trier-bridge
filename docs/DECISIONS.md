...
| DEC-019 | Delivery model: Trier Bridge ships as one integrated build. The application (launcher/search, familiar file entry points, settings routing, system tools, Bridge Terminal) and every desktop integration it provides (file-manager context-menu entries, desktop search provider, launchers) are part of the completed product, install together, and are active after installation. Integrations use only each desktop's standard extension points and augment the native file manager rather than replacing it. Per DEC-018 each integration stays individually switchable in Trier Bridge settings and uninstall removes all of them; global key/mouse rebinding and file-association takeover remain separately opt-in. Privileged operations run in a small separate system service registered with polkit, started on demand, exiting when idle. Sandboxed package formats that block system access are not candidates for the core. Owner accepted 2026-09-20; integrations are not optional add-ons. | Accepted |
