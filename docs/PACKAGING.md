...
- **Installing without a terminal** (owner requirement: the audience is a Windows user, not a power user). On Ubuntu 24.04 Desktop, App Center registers `application/vnd.debian.binary-package` and installs a local `.deb` with one Install button and the normal password prompt — but the archive viewer (File Roller) registers the same type and wins the default, so a double-click on the `.deb` opens it as an archive instead (verified on ENV-02 with the session's own `XDG_DATA_DIRS`: both are registered, File Roller is the default, no system `mimeapps.list` picks either). The double-click path is therefore: right-click the `.deb` → Open With → App Center, ticking "Always use for this file type" the first time; after that a double-click installs. Once Trier Bridge is installed, Apps → Default apps lists "Software installers (.deb)" so the same choice can be made there (DEC-028). Trier Bridge itself never installs or removes packages (DEC-024); it only routes the file type to the system's own installer.
