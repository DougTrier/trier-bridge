...
The schema of privileged operations is finite and small. Every operation is an `Operation` with a `kind` drawn from a fixed vocabulary (`trier_bridge/core/operations.py`); the only kinds that can require privilege are `service.start|stop|restart|enable|disable` on one named systemd unit (name, object path, fragment path) and, since DEC-024, `network.disconnect|connect|static|auto|dns|dns-auto` on one NetworkManager connection (UUID and interface). They are executed by one method call on `org.freedesktop.systemd1` or `org.freedesktop.NetworkManager` over the system bus with `ALLOW_INTERACTIVE_AUTHORIZATION`. polkit decides per call and binds any grant to the calling process, its session, and the action (`org.freedesktop.systemd1.manage-units`, `org.freedesktop.NetworkManager.network-control`, `org.freedesktop.NetworkManager.settings.modify.system`). Trier Bridge never holds a credential, never caches an authorization, never runs a helper, never calls `sudo`, and never runs a shell. The same plan is used whether the request came from the Services page or from `sc stop <name>` in the Bridge Terminal, and it is shown to the user and confirmed before the call is made (TB-INV-094, TB-INV-104, TB-INV-109, TB-INV-110).
