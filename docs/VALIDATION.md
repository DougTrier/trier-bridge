...
### DOC-02.01 — In-app file browser: read-only browsing, mutations, root curation

- **Timestamp:** 2026-09-21 05:31 PM CDT; updated 2026-09-21 05:40 PM CDT (Phase 2 mutations, Phase 3 root curation added — same entry, not a new one, since the claim and its gap are the same shape throughout)
- **Candidate revision:** d76d190 (filelisting.py at 1db6092; filebrowser.py/FilesPage/window.py at e8402c2, 20284ea, d76d190)
- **Environment/profile:** host Windows 11 (.venv); tb-ubuntu-desktop-2404 (Ubuntu 24.04.5, Wayland GNOME 46, Hyper-V Gen2). VM console was occupied by the owner's own hands-on testing session throughout all three rounds (`trier-bridge --section apps` running, pid 3551, unchanged across all three pushes) — the app is single-instance (TB-INV-063), so a second launch for testing would have redirected his session, not opened a separate window. Deliberately not done.
...
- **Failures/limitations:** **none of `FileBrowserPage` has been driven live, at any of the three rounds** — no AT-SPI walk, no owner click-through, no screenshot, not for browsing, not for a rename/cut/copy/paste/trash, not for the root-curation toggle. Every claim above is a static one (types check, tests pass against real fixtures, the module imports and constructs no differently from the codebase's established page pattern); none of it is evidence that the widgets render correctly, that a popover menu opens where expected, that a confirm dialog behaves modally the way `terminal.py`'s does, that the breadcrumb reflows sanely, or that the accessible labels read sensibly to a screen reader. This stays NOT_RUN for desktop behavior until the VM console is free or the owner runs it himself. Recycle Bin/Removable drives/Network still only hand off to Files (GVfs-virtual locations, not real paths `filelisting.py` can read) — an intentional, named gap. No column-header sort (by size/date) yet; sort is fixed to dirs-first/casefold, same as `cmd_dir`. Rows beyond `VISIBLE_CAP=500` in one folder are hidden behind search rather than all rendered, unverified against a real large folder. Mutations support one entry at a time — no multi-select yet, so TB-INV-235 (TOCTOU-safe multi-selection) is not yet exercised by anything real; it is not violated, since nothing multi-selects. `ROOT_SYSTEM_DIRS` is a fixed, hand-picked FHS list, not derived from anything the system reports — a nonstandard top-level folder on an unusual install would show through un-curated.
- **Evidence state:** UNIT_VERIFIED (`filelisting.py`) and TOOL_VERIFIED (all UI modules pass the full static gate on host and VM); DESKTOP_VERIFIED NOT_RUN for every behavior described above
