...
**Candidate:** commit `fb23e0f` (Foundations 01–07 closed, SCOPE-14 integrations, Foundation 08 partly qualified; package 0.1.0~dev1)  
**Timestamp:** 2026-09-21 09:55 AM CDT  
**CQS:** **NOT MEASURED**  
**Assessed weight:** 85 / 100 (seven of eight categories have current evidence)  
**Observed on assessed weight:** 79 of 85 (not normalized to 100 by rule)  
...
| Readability / naming | 15 | Trier Bridge vocabulary throughout (Operation, CapabilityState, ProcessIdentity, UnitIdentity, FileIdentity, MimeIdentity, TerminatePlan, FilePlan, ServicePlan, DefaultAppPlan, Integration) (1); accurate names (1); understandable control flow: eighteen functions above cyclomatic 10 are dispositioned below; two are flagged for splitting (0.5); consistent state/result terms matching `docs/STATE-AND-PERSISTENCE.md` (1); coherent module scope (1) | 13.5 | MEASURED |
| Complexity | 15 | cyclomatic: 18 functions above 10, maximum 20 (`plan_file`), all dispositioned below; `read_devices` reduced from 30 earlier today (0.5); cognitive complexity: no tool in the pinned toolchain, not measured (NM); nesting review: deepest nesting four levels in `execute_service` (0.5); function/module scope: longest function 131 lines (`execute_service`), largest module `ui/window.py` 460 lines (0.5); duplicated decision review: one state machine, one identity comparison per kind, one redaction function, one atomic write, one ledger, one plan→confirm→execute shape reused by four operation kinds (1) | — | **NOT MEASURED** (one criterion) |
...
| Dead code / duplication | 5 | unused paths: pyflakes clean (1); unreachable branches: mypy `warn_unreachable` clean (1); exact duplicates: none found by reading; `Paths` still names two different things (`config.Paths`, `integrations.catalog.Paths`), CQ-07 (0.5); semantic duplicates / state authority: one authority each (1); standalone foundations distinguished and labelled (1) | 4.5 | MEASURED |
...
|---|---:|---|
| `operations/files.py: plan_file` | 20 | One function validates six verbs against the filesystem (exists, is a folder, is empty, destination taken, parent missing) and produces the plain refusal for each. Every branch is one user-visible rule with a unit test. **CQ-08: split per verb** once a seventh verb arrives; kept now because the refusal wording is reviewed as a whole. |
| `operations/service.py: execute_service` | 18 | validate → revalidate → authorize → call → verify for five actions and every error class; 131 lines, four nesting levels. **CQ-05** (split into per-action verification helpers). |
...
| `system/storage.py: read_storage` | 15 | udisks2 object walk with filters. **CQ-06** (decompose per object kind as `read_devices` was). |
...
| CQ-08 | 10 complexity | Split `plan_file` per verb when the next verb is added. | open |
...
> **CQS: NOT MEASURED** (assessed weight 85 of 100; observed 79 of 85; complexity category incomplete)

This is the truthful result for candidate `fb23e0f`.
