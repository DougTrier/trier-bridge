...
| Test results | host: 105 unit passed, 11 skipped (need `gi`, CUPS, logind, or POSIX permission bits); VM: 139 unit passed, 1 skipped; integration 51 passed, 3 skipped with the journeys excluded (all 3 skips need a console session, unavailable over this SSH-only test connection; the owner's test-account password, supplied mid-session, resolved the 5 grant-path skips an earlier pass of this same run had read) — one short of candidate `87bd7d6`'s 53-passed/0-skipped run, and the shortfall is exactly the 3 console-gated tests against a total one test larger (the new boot-scoped test); all normalized by `tools/dev.py results`; bandit: 0 findings (entry IMP-07.12) |
...
| Testing / regression | 15 | tests map to invariants by `TB-T###` in docstrings (1); 140 unit tests on real files, real child processes, the real Trash, the real mimeapps.list; no mocks (1); integration/negative tests against live services: 51 passed this session in the VM with journeys excluded (systemd, polkit through a real agent, NetworkManager, udisks2, journald, sysfs, D-Bus activation, loopback ext4 faults); 3 skipped for lacking a console session over SSH, not a code gap (entry IMP-07.12) (1); failure/lifecycle/recovery cases: state-machine refusals, disk-full and kill-mid-write recovery, stale identity for four target kinds, PARTIAL restart, denied and dismissed authorization, read-only ledger (1); candidate-specific regression evidence: `docs/TEST-STRATEGY.md` section 7 maps SECURITY section 37 to tests with gaps named; same revision run on host and VM; installed `.deb` exercised through AT-SPI (1) | 15 | MEASURED |
