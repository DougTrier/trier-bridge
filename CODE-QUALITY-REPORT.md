...
**Candidate:** commit `5791bdc` (**version 1.0.0**: the no-shell terminal helper, the accent frame fixed at the container level, Task Manager Performance polish, Home's goal paragraph and search fix, the missing-icon fix; package `trier-bridge_1.0.0_all.deb`). Covers everything since `e6ad2f8`: the file browser (DEC-025), session-critical protection, Performance graphs (DEC-026), the launcher icon suite, the ping/tracert terminal fixes, the shell restyle (DEC-027), and the final polish pass (DEC-028).  
...
| `ui/chart.py: _draw` | 15 | New at `5791bdc` (entry SCOPE-16 round two): one Cairo pass that draws the grid, the gradient fill and the line per unbroken segment (a `None` sample ends a segment, TB-INV-251), and the end-point marker. Presentation only; the values drawn are exactly the samples pushed. Kept; a candidate for splitting the segment builder out if it grows again. |
