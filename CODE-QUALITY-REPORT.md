...
**Candidate:** commit `82ea597` (Foundations 01–08 automated parts, DEC-024 network translation layer and drive letters; package 0.1.0~dev1)  
**Timestamp:** 2026-09-21 04:45 PM CDT  
...
| Test results | host: 105 unit passed, 7 skipped (need `gi`, CUPS, or POSIX permission bits); VM: 125 unit passed, 1 skipped; integration 50 in the console session including 4 on a NetworkManager dummy interface; bandit: 0 findings |
...
| `bridge/cmdlets.py: translate` | 11 | Parameter grammar for cmdlets; each branch has a unit test. Kept. |
...
Both lists come from `python3 tools/dev.py complexity` in the VM (14 cyclomatic, 28 cognitive); the host mirror of the plugin reports a few fewer. Every function entering either list is dispositioned here before its ledger item closes.
...
This is the truthful result for candidate `82ea597`. It is an engineering quality score, not a release verdict: release stays locked and the hard gates above still list what is unverified.
