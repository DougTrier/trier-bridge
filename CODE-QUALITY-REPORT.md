...
**Candidate:** commit `f4042b5` (**version 1.0.0**: the no-shell terminal helper, the accent frame fixed at the container level, Task Manager Performance polish, Home's goal paragraph and search fix, the missing-icon fix; package `trier-bridge_1.0.0_all.deb`). Covers everything since `ac39ee7`: the file browser (DEC-025), session-critical protection, Performance graphs (DEC-026), the launcher icon suite, the ping/tracert terminal fixes, the shell restyle (DEC-027), and the final polish pass (DEC-028).\
**Timestamp:** 2026-09-21 11:40 PM CDT  
**CQS:** **94 / 100** (all eight categories measured; unchanged from candidates `ff5dd93` and `ac39ee7` — no criterion's 0/0.5/1 score moved; the six new complexity outliers are dispositioned below and stay within the 0.5 already recorded for those criteria)\
...
| Item | Value |
|---|---|
| Source scope | `trier_bridge/` (16,887 lines, 75 files, per `dev.py evidence`), `tests/` (unit, integration, the polkit test agent, AT-SPI journeys; 199 unit cases in the VM), `tools/dev.py`, `data/integrations/tb_nautilus.py`, `data/terminal/tb-hold-terminal.py` (the shipped terminal helper: a separate process, launched as an argument list, that runs one program as a list — no shell) |
...
| Test results | host: 143 unit passed, 17 skipped (need `gi`, CUPS, logind, or POSIX permission bits); VM: 199 unit passed, 1 skipped (up from 140 at `ac39ee7`: real-fixture tests for the file listing, disk/network sampling and the chart, the shell theme, the section table, the default-app type list, and the terminal helper run as a real child process); integration 46 passed, 8 skipped with the journeys excluded and without the test-account password in this SSH session (the same environment-limited shape as the `ac39ee7` pass; the fuller 51 passed/3 skipped run with the password stands as the best evidence, entry IMP-07.12); all normalized by `tools/dev.py results`; bandit: 0 findings (entry SCOPE-16) |
...
| Complexity | 15 | cyclomatic: 18 functions above 10, maximum 19 (`_build_page`, one more page since `ac39ee7`); three new since `ac39ee7` (`list_directory` 12, `run_in_terminal` 13, `Chart._draw` 15), every outlier dispositioned below (0.5); cognitive complexity: 35 functions above 15 (flake8-cognitive-complexity, threshold 15), four new since `ac39ee7`, maximum still 30, all dispositioned below (0.5); nesting review: deepest nesting three levels (0.5); function/module scope: longest logic function 92 lines (`Discovery.environment`; `theme.generate_css` is longer at 180 lines but is one CSS template string with no branches), largest module `bridge/commands.py` 1,549 lines (0.5); duplicated decision review: one state machine, one identity comparison per kind, one redaction function, one atomic write, one ledger, one plan→confirm→execute shape reused by four operation kinds, one CSS provider for the whole window (1) | 9 | MEASURED |
...
| Static-analysis health | 10 | build diagnostics: package builds with no warnings; lintian silent (1); lint and type analysis: flake8 and mypy strict clean (1); security/safety findings: bandit 1.6.2 over the product reports zero findings (four low-severity `assert` uses were replaced by explicit checks today); manual grep still shows zero subprocess/shell/os.system in `trier_bridge/`; the one `subprocess.run` in the tree is the shipped terminal helper under `data/terminal/`, a separate process that receives a list and never a shell string, covered by its own real child-process tests (1); suppression inventory: 173, all of three documented kinds, listed below (1); resource/nullability/unsafe/boundary warnings: mypy strict with `warn_unreachable` clean (1) | 10 | MEASURED |
...
| `system/netdiag.py: run_in_terminal` | 13 | New since `ac39ee7` (entry IMP-07.14): the terminal launch grew three real branches — the stay-open wrapper, the marker-file read that finds the pty shell to replace on the next run, and the `Gio.AppInfo` fallback when no `x-terminal-emulator` is on `PATH`. Each branch was probed live on the VM before it shipped. Kept; a candidate for splitting the marker wait into a helper if it grows again. |
| `system/filelisting.py: list_directory` | 12 | New since `ac39ee7` (entry DOC-02.01): one branch per entry kind (dir, file, symlink-to-each, broken link, FIFO, socket, device) plus the hidden-file and root-curation filters, each with a real-fixture test. Kept. |
| `ui/chart.py: _draw` | 15 | New at `f4042b5` (entry SCOPE-16 round two): one Cairo pass that draws the grid, the gradient fill and the line per unbroken segment (a `None` sample ends a segment, TB-INV-251), and the end-point marker. Presentation only; the values drawn are exactly the samples pushed. Kept; a candidate for splitting the segment builder out if it grows again. |
...
| `bridge/commands.py: cmd_tree` | 22 | Reviewed; kept. |
| `bridge/commands.py: cmd_dir` | 21 | Windows-style listing with sizes, dates, hidden filter. Kept. |
| `bridge/grammar.py: parse` | 21 | Grammar plus the cmdlet hook. Kept. |
...
| `system/netdiag.py: run_in_terminal` | 20 | New since `ac39ee7` (entry IMP-07.14); see the cyclomatic row above. Kept. |
| `ui/filebrowser.py: _render` | 19 | New since `ac39ee7` (entry DOC-02.01): one row shape per entry kind, with the per-kind icon, the root-curation hide, and the visible-row cap. Presentation only. Kept. |
...
| `ui/filebrowser.py: _on_key_pressed` | 17 | New since `ac39ee7` (entry DOC-02.01): Explorer's own key bindings (Backspace, Alt+arrows, Delete, F2, Ctrl+C/X/V), one branch per key, each calling the identical action method the row menu calls. Kept. |
...
| `ui/chart.py: _draw` | 27 | New since `ac39ee7` (entry IMP-04.01, DEC-026); grew from 16 at `f4042b5` with the per-resource gradient fill, per-segment drawing, and the end-point marker (SCOPE-16 round two). One Cairo pass; a gap for every `None` sample (TB-INV-251) rather than interpolation. Presentation only. Kept; see the cyclomatic row. |

**New since `ac39ee7`:** three cyclomatic and four cognitive outliers, all from the evening's new surfaces (file browser, performance graphs, the terminal fixes), each dispositioned above; nothing previously listed left the lists. `ui/window.py: _build_page`/`_on_row_selected` did not grow with the shell restyle because the hero, color, and zoom logic live in their own methods and `ui/theme.py`. At `f4042b5` the only movement was `Chart._draw` (cyclomatic 15, cognitive 27) for the Performance tab's per-resource gradient charts.

Both lists come from `python3 tools/dev.py evidence` in the VM (18 cyclomatic, 35 cognitive, written to `reports/local/quality-evidence.json`); the host mirror of the plugin reports a few fewer. Every function entering either list is dispositioned here before its ledger item closes.

---

## Suppression inventory (173)

Counted by `tools/dev.py evidence` in the VM for candidate `f4042b5`: 144 `# noqa` (all `E402`), 29 `type: ignore`, 0 `# nosec` — up from 122/26/0 at `ac39ee7`, all of the same three kinds: every new `E402` is a `gi.repository` import in a new or re-ordered module (`filelisting`, `filebrowser`, `diskio`, `netio`, `chart`, `theme`'s consumers, `about`, `netdiag`'s helper-path import) and every new `type: ignore[misc]` is a new `Gtk`/`Adw` subclass (`FileBrowserPage`, `Chart`, `AboutPage`). `cairo` joined `gi`/`cups` in mypy's missing-import override (`pyproject.toml`): pycairo is part of PyGObject on the target and absent on the Windows host; that is a config line, not a suppression marker.

| Kind | Count | Where | Reason |
|---|---:|---|---|
| `# noqa: E402` | 144 | every module that imports `gi.repository` | PyGObject requires `gi.require_version()` before importing `gi.repository`; imports after it are the documented pattern |
...
| Shell injection | no shell use; every launch is a fixed argument list through GLib; hostile filenames are data. The terminal hold for ping/tracert is a shipped helper run as an argument list — a `sh -c` wrapper that briefly existed on 2026-09-21 was caught by `tb terms` (TB-TERM-007) and removed the same evening | zero subprocess/shell/os.system matches in `trier_bridge/`; `tb terms` 0 errors at `f4042b5`; entries IMP-07, IMP-06.08, IMP-07.14 addendum, SCOPE-16 round two |
...
This is the truthful result for candidate `ac39ee7`, refreshed from `ff5dd93` for the `nslookup` timeout fix (evidence entry IMP-07.13) and the live desktop verification done afterward at this same candidate (section 4's page walks; the End task dialog, resolved with the owner's direct help; two concurrent instances, TB-INV-063; the PowerShell button). The score is carried forward unchanged: cyclomatic and cognitive outlier counts are identical to `ff5dd93` (`netdiag.py`'s new code stays under both thresholds), suppression counts are unchanged (122/26/0), bandit stayed at 0, and the one added unit test (140, was 139) plus the live AT-SPI confirmations strengthen rather than weaken the testing criteria already scored at 1. No criterion's disposition changed enough to move a 0/0.5/1 score, so this refresh does not re-litigate the number; a reviewer who disagrees should change the specific criterion row, not the total. It is an engineering quality score, not a release verdict: release stays locked and the hard gates above still list what is unverified.
