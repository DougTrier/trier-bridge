...
**Updated:** 2026-09-22 02:02 AM CDT
...
- Linux testing uses the project-owned Hyper-V Ubuntu Desktop VM `tb-ubuntu-desktop-2404` created by `tools/env/New-TbDesktopVm.ps1`, with this narrow owner-approved exception (2026-09-22, CI-01): GitHub-hosted disposable `ubuntu-24.04` runners may perform package builds, static/security checks, and the reviewed non-destructive unit subset in `tools/ci-unit.py`. Desktop, privileged, system-mutation, and fault-injection tests remain restricted to the project VM. Cloud build/unit results do not qualify a desktop or distribution. No self-hosted runner, package install test, package upload, signing, or release is authorized by this exception. WSL is not used for project testing. The owner may keep the VM for other uses; its removal is owner discretion.
